...
public class BeamUtilFunctions implements Serializable {
    ...
    public static ParDo.SingleOutput<StreamlineEvent, StreamlineEvent> applyAlias(String aliasName, String key) {
        return ParDo.of(new DoFn<StreamlineEvent, StreamlineEvent>() {
            @ProcessElement
            public void processElement(@Element StreamlineEvent streamlineEvent, OutputReceiver<StreamlineEvent> out) {
    ...
    public static Combine.Globally<StreamlineEvent, StreamlineEvent> applyCombiningStrategy(String fieldName, String function) {
        return AggregationFunImpl.globally(fieldName, FunctionEnum.enumOf(function).getClazz()).withoutDefaults();
    }

    //filter fields
    public static ParDo.SingleOutput<StreamlineEvent, StreamlineEvent> filterFields(String beamComponentId, Set<FieldExpression> filteredFields) {
        return ParDo.of(new DoFn<StreamlineEvent, StreamlineEvent>() {
            @ProcessElement
            public void processElement(@Element StreamlineEvent streamlineEvent, OutputReceiver<StreamlineEvent> out) {
    ...
    public static ParDo.SingleOutput<StreamlineEvent, KV<byte[], StreamlineEvent>> generateKey(String sinkId, String routingKey) {
        return ParDo.of(new DoFn<StreamlineEvent, KV<byte[], StreamlineEvent>>() {
            @ProcessElement
            public void processElement(@Element StreamlineEvent streamlineEvent, OutputReceiver<KV<byte[], StreamlineEvent>> out) {
                ...
                StreamlineEvent newEvent = StreamlineEventImpl.builder().from(streamlineEvent).dataSourceId(sinkId).build();
    ...
    public static ParDo.SingleOutput<KV<Object, StreamlineEvent>, StreamlineEvent> extractStreamlineEvents(String beamSourceId) {
        return ParDo.of(new DoFn<KV<Object, StreamlineEvent>, StreamlineEvent>() {
            @ProcessElement
            public void processElement(@Element KV<Object, StreamlineEvent> kV, OutputReceiver<StreamlineEvent> out) {
                StreamlineEvent newEvent = StreamlineEventImpl.builder().from(kV.getValue()).dataSourceId(beamSourceId).build();
                out.output(newEvent);
            }
        });
    }

    public static ParDo.SingleOutput<StreamlineEvent, StreamlineEvent> addTimestamp(String timestampField) {
        return ParDo.of(new DoFn<StreamlineEvent, StreamlineEvent>() {
            @ProcessElement
            public void processElement(@Element StreamlineEvent streamlineEvent, OutputReceiver<StreamlineEvent> out) {
