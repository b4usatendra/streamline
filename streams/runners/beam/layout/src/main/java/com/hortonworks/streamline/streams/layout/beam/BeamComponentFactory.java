...
class BeamComponentFactory {

  private static final Logger LOG = LoggerFactory.getLogger(BeamComponentFactory.class);

  private final Path extraJarsLocation;

  BeamComponentFactory(Path extraJarsLocation) {
  ...
  BeamComponent getFluxComponent(StreamlineComponent streamlineComponent) {
    ...
    try {
      BeamComponent beamComponent = proxyUtil.loadClassFromLibDirectory(extraJarsLocation,
          streamlineComponent.getTransformationClass());
